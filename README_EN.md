...
Documentation-only PRs and documentation-only protected-branch pushes retain their documentation tier. Process documents skip frontend and Rust builds; README, AGENTS, and specification changes still run documentation contracts. Frontend-only or Rust-only PRs run the affected domain; shared, mixed, or unknown paths run both. Protected-branch pushes with code or unknown files retain complete CI; CodeQL runs independently. Release tags must point to the actual main merge commit with a successful signed candidate. See [commits and releases](docs/operations/github-actions-governance.md#提交与发版).
