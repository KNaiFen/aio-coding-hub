...
| 5 | `.trellis/tasks/08-06-cross-restart-data-reset` / `codex/aud020-cross-restart-data-reset` | `AUD-008` | durable marker/tombstone、专用硬退出、启动前清理、失败 maintenance retry/exit 闸门、前端生命周期卸载。 | 当前重放至 AUD-056 主线；待完成重放后记录精确候选、运行云端跨平台生命周期门与绑定核验；为 AUD-002 提供共享 coordinator。 |
