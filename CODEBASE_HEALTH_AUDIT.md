...
- 2026-08-07 代码完成、交付待执行：依赖 AUD-008 候选的 `codex/aud021-filesystem-recovery-journal` 已提交 `eed41abb`、`23718b01`。它新增 durable recovery journal、SQLite 权威 replay、Skills 受管 artifact ownership/hash 边界、Prompt/MCP/Skills/workspace switch 的 prepare-first 协议与 maintenance retry/exit gate；错误摘要保持定长脱敏。AUD-020 合并后须重放到主线并在精确 SHA 运行全量 Actions；本地只执行零依赖合同检查与差异检查，条目继续保持 `planned`。
...
| 6 | `.trellis/tasks/08-06-filesystem-recovery-journal` / `codex/aud021-filesystem-recovery-journal` | `AUD-002` | prepare-first journal、SQLite 权威 replay、Skills 受管 artifact、补偿错误聚合。 | 正在重放到最新 AUD-008 候选；完成后以新的精确 head 运行云端故障注入/重启/脱敏/并发门与 bindings 核验。 |
