...
- 2026-08-07 代码完成、交付待执行：依赖 AUD-008 候选的 `codex/aud021-filesystem-recovery-journal` 已重放为 `0e8464c0`。它新增 durable recovery journal、SQLite 权威 replay、Skills 受管 artifact ownership/hash 边界、Prompt/MCP/Skills/workspace switch 的 prepare-first 协议与 maintenance retry/exit gate；错误摘要保持定长脱敏。AUD-020 合并后仍须重放到主线并在精确 SHA 运行全量 Actions；本地只执行零依赖合同检查与差异检查，条目继续保持 `planned`。
...
| 4 | `.trellis/tasks/08-06-gateway-lan-bearer-token` / `codex/aud019-gateway-lan-bearer-token` | `AUD-016` | 候选 `948dc5fa`：真实 peer 全路由 token、一次性展示/轮换、header 脱敏、WSL manifest v2/同步和 provider 专用路径移除。 | workflow_dispatch `31197593860` 已针对精确候选启动；云端 socket/路由/日志/WSL/bindings 门与合并待执行，不阻塞后续代码。 |
| 5 | `.trellis/tasks/08-06-cross-restart-data-reset` / `codex/aud020-cross-restart-data-reset` | `AUD-008` | 候选 `c4c069e2`：durable marker/tombstone、专用硬退出、启动前清理、失败 maintenance retry/exit 闸门、前端生命周期卸载。 | 已重放至 AUD-056 主线；待运行云端跨平台生命周期门与绑定核验，并在 AUD-016 合并后按顺序合入；为 AUD-002 提供共享 coordinator。 |
| 6 | `.trellis/tasks/08-06-filesystem-recovery-journal` / `codex/aud021-filesystem-recovery-journal` | `AUD-002` | 候选 `0e8464c0`：prepare-first journal、SQLite 权威 replay、Skills 受管 artifact、补偿错误聚合。 | 已重放到最新 AUD-008 候选；待以该精确 head 运行云端故障注入/重启/脱敏/并发门与 bindings 核验，AUD-008 合并后再重放到主线。 |
