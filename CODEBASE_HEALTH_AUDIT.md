...
- 2026-08-07 代码完成、交付待执行：独立候选分支 `codex/aud022-observer-zero-history-query` 已重放为 `dc9a4418`，实现受限 last/dominant/recent 查询、zero-history ready-empty 投影、Claude 可见性过滤与 source-aware 有界 folder cache。缓存命中不占 miss 扫描预算，`history_limit=0` 不调用 recent 查询；本地仅执行 cloud-only checker/self-test 与 `git diff --check`，完整 Rust 查询/缓存回归、bindings 与合并仍由 Actions 验证，条目保持 `planned`。
...
| 7 | `.trellis/tasks/08-06-observer-zero-history-query` / `codex/aud022-observer-zero-history-query` | `AUD-035` | 候选 `dc9a4418`：受限 last/dominant/recent 查询、zero-history ready-empty 与 source-aware 有界 folder cache。 | 已重放到 AUD-056 主线；待以该精确 head 运行查询/缓存回归与 bindings 门，并在前序顺序任务合并后交付。 |
