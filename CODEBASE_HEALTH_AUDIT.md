...
| AUD-008 | `planned` | `P1` | Rust 数据重置 / 资源生命周期 | 数据重置未停止长期持有 DB pool 的后台任务，就按非原子顺序删除设置与 SQLite 文件 | 候选 `c4c069e2`；`.trellis/tasks/08-06-cross-restart-data-reset`；`src-tauri/src/app/maintenance.rs`、`src-tauri/src/commands/data_management.rs` |
...
- 2026-08-08 代码完成、主线重放待交付：独立候选分支 `codex/aud022-observer-zero-history-query` 已基于 `origin/main@99de56bb` 重放，产品候选为 `be295bdb`，实现受限 last/dominant/recent 查询、zero-history ready-empty 投影、Claude 可见性过滤与 source-aware 有界 folder cache。缓存命中不占 miss 扫描预算，`history_limit=0` 不调用 recent 查询；本地仅执行 cloud-only checker/self-test 与 `git diff --check`。前序 PR #92 的全量 run `31245416972` 因 generated-file drift 失败并已形成精确补丁，不阻塞本项；完整 Rust 查询/缓存回归、bindings 与合并仍由 Actions 验证，条目保持 `planned`。
...
| 4 | `.trellis/tasks/08-06-gateway-lan-bearer-token` / `codex/aud019-gateway-lan-bearer-token` | `AUD-016` | 候选 `948dc5fa`：真实 peer 全路由 token、一次性展示/轮换、header 脱敏、WSL manifest v2/同步和 provider 专用路径移除。 | workflow_dispatch `31197593860` 已针对精确候选启动；云端 socket/路由/日志/WSL/bindings 门与合并待执行。 |
| 5 | `.trellis/tasks/08-06-cross-restart-data-reset` / `codex/aud020-cross-restart-data-reset` | `AUD-008` | 候选 `c4c069e2`：durable marker/tombstone、专用硬退出、启动前清理、失败 maintenance retry/exit 闸门、前端生命周期卸载。 | workflow_dispatch `31198507547` 已针对精确候选启动；跨平台生命周期门、bindings 核验与顺序合并待执行。 |
| 6 | `.trellis/tasks/08-06-filesystem-recovery-journal` / `codex/aud021-filesystem-recovery-journal` | `AUD-002` | 候选 `0e8464c0`：prepare-first journal、SQLite 权威 replay、受管 artifact 和补偿错误聚合。 | workflow_dispatch `31199435741` 已针对精确候选启动；AUD-008 合并后重放到主线。 |
| 7 | `.trellis/tasks/08-06-observer-zero-history-query` / `codex/aud022-observer-zero-history-query` | `AUD-035` | 产品候选 `be295bdb`：受限 last/dominant/recent 查询、zero-history ready-empty 与 source-aware 有界 folder cache。 | 已重放到 `origin/main@99de56bb`；本地 cloud-only 检查通过，待以 PR #93 的精确交付 head 运行查询/缓存回归与 bindings 门。前序 #92 CI 漂移不阻塞本项。 |
