...
同步策略检查器只接受当前批准的六个有序步骤，并把 App token 的消费者限定为 checkout 与 `GH_TOKEN`。已有同步 PR 时，只能执行一次以当前仓库、上游 head、目标 base 与 open 状态限定的 `gh pr list` 查询；新建 PR 时必须捕获 `gh pr create` 的 stdout，严格接受当前仓库的 `https://github.com/${GITHUB_REPOSITORY}/pull/<正整数>` URL 并直接取出编号，不能再 list、猜测或重试。URL、编号或 merge state 异常均 fail-closed，且不得 push、merge 或自动批准。凭据预检、fetch 和 PR 创建脚本是完整正文合同；调整这些步骤时必须同时更新 policy selftest，不能用附加 action 或间接 Shell 调用扩展写权限。
