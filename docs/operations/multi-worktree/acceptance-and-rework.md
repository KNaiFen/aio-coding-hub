...
3. 独立验收者只读审查固定 head 的 diff 与交付证据，不执行候选脚本，不接受 head 漂移或 policy/bundle 不一致。
4. 结论必须绑定同一完整 head；新提交立即使结论失效。

## 返工

- 有阻塞 finding 时保持任务为 `delivered`/`rework`，不得合并。
- trusted main 传入 delivered head、PR snapshot、review、candidate worktree 和 runtime root，生成全新 rework offer。
- rework 会撤销旧 offer；禁止复用旧 claim、activation、receipt 或手写状态。
