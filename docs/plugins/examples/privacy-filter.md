...
Gateway boundary note：gateway 会先接收原始 client-to-gateway body；Privacy Filter 仅在 body 不超过 1 MiB plugin visible-context budget 且非法 UTF-8 替换后的可见字符串也未超过该上限时接收完整 request body。它的保护保证是：当插件启用、hook 成功执行且选中匹配策略和处理范围后，gateway-to-upstream provider request body 中的白名单字段和 persisted request logs 会被脱敏。超过该 budget 的 request body 或 UTF-8 replacement expansion 会标记为 truncated；由于官方 request hooks 是 `fail-closed`，宿主会在 Extension Host 执行前返回 `PLUGIN_CONTEXT_TRUNCATED`，不向 upstream 发送未扫描的尾部，也不把该输入规模拒绝计入插件 circuit。日志脱敏由 `redactLogs` 和 `sensitiveTypes` 控制，不受 request `redactionScopes` 影响。如果你检查 hook 执行前的本地 client request，仍可能看到原始输入。官方 manifest 将 `log.beforePersist` 声明为 `fail-closed`；该 hook 执行失败、circuit-open、visible log message 被截断或返回非法最终 payload 时，宿主会保留插件诊断并跳过整条 request log，不会把原始日志回退入库。故障期间会出现请求历史缺口。

Official privacy filter rules are loaded under a separate 1 MiB host byte budget。`official.privacy-filter` 通过 `api.privacy.redactRequestBody` 和 `api.privacy.redactText` 调用宿主脱敏服务，并在 manifest 的 gateway hooks 上声明 `timeoutMs: 5000`，避免大 payload 脱敏被默认轻量 hook 预算误杀；community redaction plugins should use Extension Host gateway hooks and ordinary host APIs, and may declare their own positive `timeoutMs` when needed.
