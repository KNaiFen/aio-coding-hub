...
- 当前 `log.beforePersist` 默认 fail-open；hook 失败、超时或返回非法 payload 时保留原始日志继续入库。显式 `fail-closed` 的日志 hook 在失败、熔断或最终 payload 无法安全解析时会放弃该条请求日志，并保留插件诊断；它不会回退写入原始敏感日志。
