...
撤销或宿主判定危险的插件会进入 `quarantined`。隔离插件不能启用；用户需要卸载、回滚到可用版本，或等待可信来源发布新版本。Market 来源的运行时隔离在无法复核当前签名市场状态时保持失败关闭，不会仅凭本地包完整性恢复。隔离和回滚会保留 audit 记录，便于追踪生命周期状态变化。
...
- 一个未命中 fixture。
- 一个边界或失败策略 fixture。
- 对应的宿主运行报告、导出的 replay fixture 或 CI 检查。
