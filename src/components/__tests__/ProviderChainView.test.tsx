...
describe("components/ProviderChainView", () => {
  it("keeps a rejected HTTP 200 distinct from final success and preserves generic diagnostics", () => {
    const rejected = {
      provider_id: 1,
      provider_name: "A",
      base_url: "https://a",
      ...
      status: 200,
      error_code: GatewayErrorCodes.RESPONSE_REJECTED,
      decision: "switch",
      plugin_decision: {
        plugin_id: "community.tail-policy",
        reason_code: "custom.tail_rejected",
        message: "尾部标记未通过",
      },
    };
    ...
    expect(screen.getByText("共尝试 2 次")).toBeInTheDocument();
    expect(screen.getAllByText("HTTP 200")).toHaveLength(2);
    expect(screen.getAllByText("请求成功")).toHaveLength(1);
    expect(screen.getByText("响应被插件策略拒绝")).toBeInTheDocument();
    expect(screen.getByText("community.tail-policy · custom.tail_rejected")).toBeInTheDocument();
    expect(screen.getByText("尾部标记未通过")).toBeInTheDocument();
    expect(screen.queryByText("熔断器:")).not.toBeInTheDocument();
    rerender(<ProviderChainView attemptLogs={[]} attemptsJson={JSON.stringify([rejected])} />);
    expect(screen.getByText("最终失败")).toBeInTheDocument();
    expect(screen.queryByText("请求成功")).not.toBeInTheDocument();
    rerender(
      <ProviderChainView
        attemptLogs={[]}
        attemptsJson={JSON.stringify([
  ...
  it("merges policy diagnostics into compatibility attempt rows without marking HTTP 200 successful", () => {
    const rejected = {
      provider_id: 1,
    ...
    const accepted = {
      ...
      outcome: "success",
      status: 200,
      attempt_started_ms: 30,
      attempt_duration_ms: 20,
    };
    const attemptLogs: RequestAttemptLog[] = [rejected, accepted].map((attempt, index) => ({
      ...attempt,
      id: index + 1,
    ...
    expect(screen.getAllByText("HTTP 200")).toHaveLength(2);
    expect(screen.getByText("响应被插件策略拒绝")).toBeInTheDocument();
    expect(screen.getByText("community.tail-policy · tail.rejected")).toBeInTheDocument();
  });
