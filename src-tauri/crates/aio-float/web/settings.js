...
async function load() {
  try {
    ...
    element('token').placeholder = hasToken ? '已保存' : '';
...
element('connection').addEventListener('submit', async (event) => {
  ...
  element('connection-error').classList.remove('success');
  try {
    ...
    element('connection-error').classList.add('success');
...
element('appearance').addEventListener('submit', async (event) => {
  ...
  element('appearance-error').classList.remove('success');
  try {
    ...
    element('appearance-error').classList.add('success');
