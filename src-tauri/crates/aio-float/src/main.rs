...
use tauri::{menu::{CheckMenuItem, Menu, MenuItem, PredefinedMenuItem}, tray::TrayIconBuilder, Emitter, Manager};
...
fn float_settings(app: tauri::AppHandle) -> Result<Settings, String> {
    ...
    Ok(Settings { config: state.config.clone(), has_token: state.dashboard.client.is_some(), error: state.dashboard.error.clone() })
}

#[tauri::command]
async fn float_connect(app: tauri::AppHandle, ip: String, port: u16, token: String) -> Result<(), String> {
    let gate = app.state::<ConnectionGate>();
    let _guard = gate.0.lock().await;
    let config = {
        let state = app.state::<Mutex<AppState>>();
        let state = state.lock().map_err(|_| "悬浮窗状态不可用")?;
        Config { ip, port, ..state.config.clone() }
    };
    config.validate()?;
    let credential_config = config.clone();
    let token = tauri::async_runtime::spawn_blocking(move || {
        if token.is_empty() { credential_config.credential()?.get_password().map_err(|_| "请输入访问令牌".to_string()) } else { Ok(token) }
    }).await.map_err(|_| "无法读取访问令牌")??;
    let client = ObserverClient::remote(&config.ip, config.port, &token).map_err(|_| "连接参数无效")?;
    let scope = aio_observer_protocol::CliScope::parse(&config.scope).unwrap_or(aio_observer_protocol::CliScope::Codex);
    client.snapshot(scope, 0).await.map_err(|reason| if reason == aio_tui::client::OfflineReason::Unauthorized { "认证失败，请检查访问令牌".to_string() } else { reason.label().to_string() })?;
    let credential_config = config.clone();
    tauri::async_runtime::spawn_blocking(move || credential_config.credential()?.set_password(&token).map_err(|_| "无法保存至系统凭据存储".to_string())).await.map_err(|_| "无法保存访问令牌")??;
    {
        let state = app.state::<Mutex<AppState>>();
        let mut state = state.lock().map_err(|_| "悬浮窗状态不可用")?;
        let next = Config { ip: config.ip, port: config.port, ..state.config.clone() };
...
#[tauri::command]
fn float_appearance(app: tauri::AppHandle, font_size: f64, background: String, opacity: f64, always_on_top: bool, click_through: bool) -> Result<(), String> {
    let state = app.state::<Mutex<AppState>>();
    let mut state = state.lock().map_err(|_| "悬浮窗状态不可用")?;
    let next = Config { font_size, background, opacity, always_on_top, click_through, ..state.config.clone() };
    next.validate()?;
    let window = app.get_webview_window("main").ok_or("窗口不存在")?;
    window.set_always_on_top(next.always_on_top).map_err(|_| "无法更改置顶状态")?;
...
fn reveal(app: &tauri::AppHandle, interactive: bool) -> Result<(), String> {
    let window = app.get_webview_window("main").ok_or("窗口不存在")?;
    if interactive {
        window.set_ignore_cursor_events(false).map_err(|_| "无法取消鼠标穿透")?;
        let state = app.state::<Mutex<AppState>>();
        let mut state = state.lock().map_err(|_| "悬浮窗状态不可用")?;
...
async fn float_open_settings(app: tauri::AppHandle) -> Result<(), String> {
    ...
    if let Some(window) = app.get_webview_window("settings") {
        ...
        tauri::WebviewWindowBuilder::new(&app, "settings", tauri::WebviewUrl::App("settings.html".into()))
            .title("AIO Float 设置").inner_size(420.0, 600.0).min_inner_size(340.0, 420.0)
            .always_on_top(true).center().build().map_err(|_| "无法创建设置窗口")?;
    }
    Ok(())
}
...
fn menu(app: &tauri::AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    Menu::with_items(app, &[
        ...
        &CheckMenuItem::with_id(app, "top", "始终置顶", true, state.config.always_on_top, None::<&str>)?,
        &CheckMenuItem::with_id(app, "through", "鼠标穿透", true, state.config.click_through, None::<&str>)?,
...
fn run_menu(app: &tauri::AppHandle, id: &str) -> Result<(), String> {
    match id {
        ...
        "settings" => {
            let app = app.clone();
            tauri::async_runtime::spawn(async move { if let Err(error) = float_open_settings(app.clone()).await { let _ = app.emit("float-error", error); } });
        }
        "hide" => {
        ...
        "top" | "through" => {
            ...
            float_appearance(app.clone(), config.font_size, config.background, config.opacity,
                if id == "top" { !config.always_on_top } else { config.always_on_top },
                if id == "through" { !config.click_through } else { config.click_through })?;
        }
        _ => {}
    }
    update_tray(app)
...
fn main() {
    tauri::Builder::default()
        .plugin(tauri_plugin_single_instance::init(|app, _, _| { let _ = reveal(app, true); }))
        ...
        .setup(|app| {
            ...
            app.manage(Mutex::new(AppState { config: config.clone(), path, dashboard, dirty_at: None }));
            ...
            if let (Some(x), Some(y)) = (config.x, config.y) { window.set_position(tauri::LogicalPosition::new(x,y))?; }
            window.set_always_on_top(config.always_on_top)?;
            let mut tray = TrayIconBuilder::with_id("float").tooltip("AIO Float").menu(&menu(app.handle())?).show_menu_on_left_click(true);
            ...
            window.on_window_event(move |event| {
                if matches!(event, tauri::WindowEvent::Moved(_) | tauri::WindowEvent::Resized(_)) {
                    if let Some(window) = handle.get_webview_window("main") {
                        if let (Ok(scale), Ok(position), Ok(size)) = (window.scale_factor(), window.outer_position(), window.inner_size()) {
                            if size.width > 0 && size.height > 0 {
                                if let Ok(mut state) = handle.state::<Mutex<AppState>>().lock() {
                                    state.config.x = Some(f64::from(position.x) / scale); state.config.y = Some(f64::from(position.y) / scale);
                                    state.config.width = f64::from(size.width) / scale; state.config.height = f64::from(size.height) / scale;
                                    state.dirty_at = Some(Instant::now());
                                }
                            }
                        }
            ...
            if needs_settings {
                let handle = app.handle().clone();
                tauri::async_runtime::spawn(async move { let _ = float_open_settings(handle).await; });
            }
            Ok(())
        })
        .on_menu_event(|app, event| { if let Err(error) = run_menu(app, event.id.as_ref()) { let _ = app.emit("float-error", error); } })
        .build(tauri::generate_context!())
        .expect("AIO Float startup failed")
        .run(|app, event| {
            if matches!(event, tauri::RunEvent::Exit) {
                if let Ok(state) = app.state::<Mutex<AppState>>().lock() { let _ = config::save(&state.path, &state.config); }
            }
        });
}
