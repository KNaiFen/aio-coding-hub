...
"#,
    ...
    fn historical_backfill_uses_reference_and_recorded_multiplier_only() {
        ...
        crate::model_prices::upsert(&db, "codex", "gpt-test", r#"{"input_cost_per_token":0.01}"#).unwrap();
        let conn = db.open_connection().unwrap();
        insert_backfill_candidate(&conn, "history-missing", "codex");
        insert_backfill_candidate(&conn, "history-zero", "codex");
        conn.execute("UPDATE request_logs SET cost_multiplier = 0.8 WHERE trace_id = 'history-missing'", []).unwrap();
        conn.execute("UPDATE request_logs SET cost_usd_femto = 0 WHERE trace_id = 'history-zero'", []).unwrap();
        for _ in 0..2 {
            backfill_missing_for_cli_with_aliases(&db, "codex", 5000, &model_price_aliases::ModelPriceAliasesV1::default()).unwrap();
        }
        let values: Vec<Option<i64>> = conn.prepare("SELECT cost_usd_femto FROM request_logs ORDER BY trace_id").unwrap()
