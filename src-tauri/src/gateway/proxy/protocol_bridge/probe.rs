...
pub(crate) enum ProbeProtocol { Responses, Chat, Anthropic, Gemini }
...
fn no_error(value: &Value) -> Result<(), &'static str> {
    if !value.is_object() { return Err("PROBE_STRUCTURE"); }
...
fn answer_blocks(value: &Value, kind: &str) -> Result<bool, &'static str> {
    ...
    for part in value.as_array().ok_or("PROBE_STRUCTURE")? {
        no_error(part)?;
        if part["type"] == kind {
            if !part["text"].is_string() { return Err("PROBE_STRUCTURE"); }
...
pub(crate) fn validate_json(protocol: ProbeProtocol, value: &Value) -> Result<(), &'static str> {
    ...
    let answered = match protocol {
        ProbeProtocol::Responses => {
            if value["object"] != "response" { return Err("PROBE_STRUCTURE"); }
            ...
            for item in value["output"].as_array().ok_or("PROBE_STRUCTURE")? {
                ...
                if item["type"] == "message" {
                    if item["role"] != "assistant" { return Err("PROBE_STRUCTURE"); }
        ...
        ProbeProtocol::Chat => {
            if value["object"] != "chat.completion" { return Err("PROBE_STRUCTURE"); }
            ...
            for choice in choices {
                ...
                if choice["message"]["role"] != "assistant" { return Err("PROBE_STRUCTURE"); }
                if !choice["message"]["content"].is_null() && !choice["message"]["content"].is_string() {
                    return Err("PROBE_STRUCTURE");
                }
        ...
        ProbeProtocol::Anthropic => {
            if value["type"] != "message" || value["role"] != "assistant" { return Err("PROBE_STRUCTURE"); }
            finish(value["stop_reason"].as_str(), &["end_turn", "stop_sequence", "max_tokens"])?;
            answer_blocks(&value["content"], "text")?
        }
        ProbeProtocol::Gemini => {
            if value.pointer("/promptFeedback/blockReason").is_some() { return Err("PROBE_TERMINATION"); }
    ...
    if answered { Ok(()) } else { Err("PROBE_NO_TEXT") }
}

#[derive(Default)]
struct Choice { assistant: bool, text: bool, finished: bool }
...
impl ProbeStream {
    pub(crate) fn new(protocol: ProbeProtocol) -> Self {
        Self { protocol, response_id: None, choices: HashMap::new(), text_blocks: HashSet::new(), open_blocks: HashSet::new(),
            anthropic_started: false, anthropic_text: false, anthropic_finished: false, terminal: false }
    }

    pub(crate) fn terminal(&self) -> bool { self.terminal }

    pub(crate) fn frame(&mut self, bytes: &[u8]) -> Result<(), &'static str> {
        let frame = std::str::from_utf8(bytes).map_err(|_| "PROBE_STRUCTURE")?;
        let data = frame.lines().filter_map(|line| line.strip_prefix("data:")).collect::<Vec<_>>();
        ...
        if data.len() == 1 && data[0].trim() == "[DONE]" {
            if self.protocol == ProbeProtocol::Chat {
                ...
                if !self.choices.values().any(|choice| choice.assistant && choice.text) {
                    return Err("PROBE_NO_TEXT");
                }
                self.terminal = true;
                return Ok(());
            }
            return if self.terminal { Ok(()) } else { Err("PROBE_UNFINISHED") };
        }
        let (event, value) = crate::gateway::proxy::sse::parse_sse_frame(frame).ok_or("PROBE_STRUCTURE")?;
        no_error(&value)?;
        if event == "error" { return Err("PROBE_UPSTREAM_ERROR"); }
        if self.terminal { return Err("PROBE_STRUCTURE"); }
        match self.protocol {
            ProbeProtocol::Responses => {
                if !event.starts_with("response.") { return Err("PROBE_STRUCTURE"); }
                if value.get("type").is_some_and(|value| value.as_str() != Some(event.as_str())) {
                    return Err("PROBE_STRUCTURE");
                }
                if let Some(response) = value.get("response") { no_error(response)?; }
                if let Some(id) = value.get("response_id").and_then(Value::as_str) {
                    if self.response_id.as_deref().is_some_and(|current| current != id) { return Err("PROBE_STRUCTURE"); }
                    self.response_id = Some(id.to_string());
                }
                if matches!(event.as_str(), "response.failed" | "response.cancelled") { return Err("PROBE_UPSTREAM_ERROR"); }
                if event == "response.created" || event == "response.in_progress" ||
                    event == "response.completed" || event == "response.incomplete" {
                    let response = value.get("response").ok_or("PROBE_STRUCTURE")?;
                    let id = response["id"].as_str().filter(|id| !id.is_empty()).ok_or("PROBE_STRUCTURE")?;
                    if self.response_id.as_deref().is_some_and(|current| current != id) { return Err("PROBE_STRUCTURE"); }
                    self.response_id = Some(id.to_string());
                    if matches!(event.as_str(), "response.completed" | "response.incomplete") {
                        if response["status"].as_str() != event.strip_prefix("response.") { return Err("PROBE_STRUCTURE"); }
            ...
            ProbeProtocol::Chat => {
                if value["object"] != "chat.completion.chunk" { return Err("PROBE_STRUCTURE"); }
                for value in value["choices"].as_array().ok_or("PROBE_STRUCTURE")? {
                    ...
                    if choice.finished && !delta.is_empty() { return Err("PROBE_STRUCTURE"); }
                    if let Some(role) = delta.get("role") {
                        if role != "assistant" { return Err("PROBE_STRUCTURE"); }
            ...
            ProbeProtocol::Anthropic => {
                if value.get("type").is_some_and(|kind| kind.as_str() != Some(event.as_str())) {
                    return Err("PROBE_STRUCTURE");
                }
                match event.as_str() {
                "message_start" => {
                    ...
                    no_error(message)?;
                    if self.anthropic_started || message["type"] != "message" || message["role"] != "assistant" {
                        return Err("PROBE_STRUCTURE");
                    }
                    self.anthropic_started = true;
                },
                "content_block_start" => {
                    if !self.anthropic_started { return Err("PROBE_STRUCTURE"); }
                    let index = value["index"].as_u64().ok_or("PROBE_STRUCTURE")?;
                    no_error(&value["content_block"])?;
                    if !self.open_blocks.insert(index) { return Err("PROBE_STRUCTURE"); }
                    if value["content_block"]["type"] == "text" {
                        if !value["content_block"]["text"].is_string() { return Err("PROBE_STRUCTURE"); }
                ...
                "content_block_delta" => {
                    let index = value["index"].as_u64().ok_or("PROBE_STRUCTURE")?;
                    if !self.open_blocks.contains(&index) { return Err("PROBE_STRUCTURE"); }
                    no_error(&value["delta"])?;
                    if value["delta"]["type"] == "text_delta" {
                        if !self.text_blocks.contains(&index) { return Err("PROBE_STRUCTURE"); }
                        if !value["delta"]["text"].is_string() { return Err("PROBE_STRUCTURE"); }
                        self.anthropic_text |= text(&value["delta"]["text"]);
                    }
                },
                "message_delta" => {
                    if !self.anthropic_started { return Err("PROBE_STRUCTURE"); }
                    if !self.open_blocks.is_empty() { return Err("PROBE_UNFINISHED"); }
                    no_error(&value["delta"])?;
                    finish(value["delta"]["stop_reason"].as_str(), &["end_turn", "stop_sequence", "max_tokens"])?;
                    self.anthropic_finished = true;
                },
                "message_stop" => {
                    if !self.anthropic_finished { return Err("PROBE_UNFINISHED"); }
                    if !self.anthropic_text { return Err("PROBE_NO_TEXT"); }
                    self.terminal = true;
                },
                "content_block_stop" => {
                    let index = value["index"].as_u64().ok_or("PROBE_STRUCTURE")?;
                    if !self.open_blocks.remove(&index) { return Err("PROBE_STRUCTURE"); }
                },
                "ping" => {},
            ...
            ProbeProtocol::Gemini => {
                ...
                for candidate in value["candidates"].as_array().ok_or("PROBE_STRUCTURE")? {
                    ...
                    let choice = self.choices.entry(index).or_default();
                    if !candidate["content"].is_null() {
                        if choice.finished { return Err("PROBE_STRUCTURE"); }
                ...
                if !self.choices.is_empty() && self.choices.values().all(|choice| choice.finished) {
                    if !self.choices.values().any(|choice| choice.text) { return Err("PROBE_NO_TEXT"); }
                    self.terminal = true;
                }
            }
        }
...
pub(crate) fn gemini_oauth_frame(stream: &mut ProbeStream, bytes: &[u8]) -> Result<(), &'static str> {
    let frame = std::str::from_utf8(bytes).map_err(|_| "PROBE_STRUCTURE")?;
    if !frame.lines().any(|line| line.starts_with("data:")) { return Ok(()); }
...
mod tests {
    ...
    fn original_json_protocols_require_text_and_explicit_supported_termination() {
        for value in [responses("completed", "OK"), responses("incomplete", "partial")] {
            assert_eq!(validate_json(ProbeProtocol::Responses, &value), Ok(()));
        }
        for status in ["completed", "incomplete"] {
            assert_eq!(validate_json(ProbeProtocol::Responses, &responses(status, " \n")), Err("PROBE_NO_TEXT"));
        }
        let mut invalid = responses("incomplete", "OK");
        invalid["incomplete_details"]["reason"] = json!("content_filter");
        assert_eq!(validate_json(ProbeProtocol::Responses, &invalid), Err("PROBE_TERMINATION"));
        invalid = responses("completed", "OK");
        invalid["output"][0]["type"] = json!("reasoning");
        assert_eq!(validate_json(ProbeProtocol::Responses, &invalid), Err("PROBE_NO_TEXT"));
        ...
        for reason in ["STOP", "MAX_TOKENS"] {
            let mut value = json!({"candidates":[{"finishReason":reason,"content":{"parts":[{"text":"OK"}]}}]});
            assert_eq!(validate_json(ProbeProtocol::Gemini, &value), Ok(()));
            value["candidates"][0]["content"]["parts"][0]["thought"] = json!(true);
            assert_eq!(validate_json(ProbeProtocol::Gemini, &value), Err("PROBE_NO_TEXT"));
        }
        for protocol in [ProbeProtocol::Responses, ProbeProtocol::Chat, ProbeProtocol::Anthropic, ProbeProtocol::Gemini] {
    ...
    fn responses_sse_is_chunk_and_utf8_boundary_independent_and_requires_matching_terminal() {
        ...
        let mut stream = ProbeStream::new(ProbeProtocol::Responses);
        stream.frame(b"event: response.created\ndata: {\"response\":{\"id\":\"other\"}}\n\n").unwrap();
        assert_eq!(stream.frame(format!("event: response.incomplete\ndata: {{\"response\":{value}}}\n\n").as_bytes()), Err("PROBE_STRUCTURE"));
        assert!(ProbeStream::new(ProbeProtocol::Responses).frame(b"data: [DONE]\n\n").is_err());
    }
    ...
    fn chat_stream_requires_same_choice_answer_finish_and_done() {
        ...
        assert!(stream.terminal());
        assert_eq!(stream.frame(b"data: {\"error\":{\"message\":\"late error\"}}\n\n"), Err("PROBE_UPSTREAM_ERROR"));
    }
    ...
    fn anthropic_stream_requires_text_block_stop_reason_and_message_stop() {
        ...
        assert!(!stream.terminal());
        stream.frame(b"event: message_stop\ndata: {\"type\":\"message_stop\"}\n\n").unwrap();
        assert!(stream.terminal());
    }

    #[test]
    fn streaming_errors_and_gemini_wrapper_do_not_become_answers() {
        for frame in [b"data: {broken}\n\n".as_slice(), b"event: error\ndata: {}\n\n", b"data: [DONE]\n\n"] {
            assert!(ProbeStream::new(ProbeProtocol::Anthropic).frame(frame).is_err());
        }
        let mut stream = ProbeStream::new(ProbeProtocol::Gemini);
    ...
    fn answer_fields_cannot_hide_errors_or_invalid_structure() {
        let cases = [
            (ProbeProtocol::Responses, json!({"object":"response","status":"completed","output":[{"type":"message","role":"assistant","content":[{"type":"output_text","text":"OK","error":"failed"}]}]})),
            (ProbeProtocol::Responses, json!({"object":"response","status":"completed","output":[{"type":"reasoning","summary":[{"text":"OK"}]}]})),
            (ProbeProtocol::Chat, json!({"object":"chat.completion","choices":[{"message":{"role":"assistant","content":null,"tool_calls":[{"function":{"arguments":"OK"}}]},"finish_reason":"stop"}]})),
            (ProbeProtocol::Chat, json!({"object":"chat.completion","choices":[{"message":{"role":"assistant","content":"OK","error":"failed"},"finish_reason":"stop"}]})),
            (ProbeProtocol::Anthropic, json!({"type":"message","role":"assistant","content":[{"type":"thinking","thinking":"OK"}],"stop_reason":"max_tokens"})),
            (ProbeProtocol::Anthropic, json!({"type":"message","role":"assistant","content":[{"type":"text","text":42}],"stop_reason":"end_turn"})),
            (ProbeProtocol::Gemini, json!({"candidates":[{"finishReason":"MAX_TOKENS","content":{"parts":[{"functionCall":{"args":{"text":"OK"}}}]}}]})),
            (ProbeProtocol::Gemini, json!({"candidates":[{"finishReason":"STOP","content":{"parts":[{"text":"OK","error":"failed"}]}}]})),
        ];
        for (protocol, value) in cases {
            assert!(validate_json(protocol, &value).is_err(), "{protocol:?}: {value}");
        }
        assert_eq!(gemini_oauth_response(&json!({"error":"failed","response":{"candidates":[]}})), Err("PROBE_UPSTREAM_ERROR"));
        assert_eq!(gemini_oauth_response(&json!({"candidates":[]})), Err("PROBE_STRUCTURE"));
    }
    ...
    fn streams_reject_text_then_errors_missing_terminals_and_token_only_output() {
        ...
        assert_eq!(responses_stream.frame(b"data: {\"type\":\"response.failed\",\"response\":{\"error\":\"failed\"}}\n\n"), Err("PROBE_UPSTREAM_ERROR"));
        for status in ["completed", "incomplete"] {
            let response = responses(status, " ");
            assert_eq!(ProbeStream::new(ProbeProtocol::Responses).frame(
        ...
        assert_eq!(gemini.frame(b"data: {\"candidates\":[{\"index\":0,\"finishReason\":\"SAFETY\"}]}\n\n"), Err("PROBE_TERMINATION"));
    }
    ...
    fn independent_choices_cannot_combine_answer_and_termination() {
        ...
        assert_eq!(validate_json(ProbeProtocol::Chat, &chat), Err("PROBE_UNFINISHED"));
        let gemini = json!({"candidates":[
            {"content":{"parts":[{"text":"OK"}]}},
            {"content":{"parts":[]},"finishReason":"STOP"}]});
        assert_eq!(validate_json(ProbeProtocol::Gemini, &gemini), Err("PROBE_UNFINISHED"));
    }
}
