...
pub(crate) const ACTIVE_HOOKS: &[HookContract] = &[
    ...
    HookContract {
        id: "gateway.response.beforeCommit",
        phase: "after complete upstream response and before any downstream commit",
        kind: HookKind::Response,
        status: HookStatus::Active,
        default_failure_policy: "fail-closed",
        timeout_ms: DEFAULT_HOOK_TIMEOUT_MS,
        reserved_header_policy: RESERVED_HEADER_POLICY,
        read_permissions: &[
            "request.meta.read",
