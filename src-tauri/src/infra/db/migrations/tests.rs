...
fn migrate_v54_to_v55_adds_independent_constrained_resets_without_touching_history() {
    ...
    let count: i64 = conn
        .query_row("SELECT COUNT(*) FROM provider_limit_resets", [], |row| {
            row.get(0)
        })
        .unwrap();
    assert_eq!(count, 0);
    for period in ["5h", "daily", "weekly", "monthly"] {
        conn.execute(
            "INSERT INTO provider_limit_resets VALUES(7, ?1, 100, 1)",
            [period],
        )
        .unwrap();
    }
    assert!(conn
        .execute(
            "INSERT INTO provider_limit_resets VALUES(7, 'total', 100, 1)",
            []
        )
        .is_err());
    assert!(conn
        .execute(
            "INSERT INTO provider_limit_resets VALUES(8, 'daily', 100, 1)",
            []
        )
        .is_err());
    assert!(conn
        .execute(
            "UPDATE provider_limit_resets SET request_log_id_cutoff=-1",
            []
        )
        .is_err());
    v54_to_v55::ensure_provider_limit_resets(&conn).unwrap();
    conn.execute(
        "UPDATE provider_limit_resets SET reset_at=NULL WHERE period='daily'",
        [],
    )
    .unwrap();
    assert_eq!(
        conn.query_row(
            "SELECT request_log_id_cutoff FROM provider_limit_resets WHERE period='daily'",
            [],
            |row| row.get::<_, i64>(0)
        )
        .unwrap(),
    ...
    conn.execute("DELETE FROM providers WHERE id=7", [])
        .unwrap();
    assert_eq!(
        conn.query_row("SELECT COUNT(*) FROM provider_limit_resets", [], |row| row
            .get::<_, i64>(
            0
        ))
        .unwrap(),
    ...
    assert_eq!(
        conn.query_row("SELECT cost FROM request_logs", [], |row| row
            .get::<_, i64>(0))
            .unwrap(),
        123
    );
