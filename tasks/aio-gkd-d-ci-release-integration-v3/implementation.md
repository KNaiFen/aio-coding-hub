...
- When changing job dependencies, update `scripts/check-ci-quality-gates.selftest.mjs` so its fixtures assert the new contract, then run the selftest through the registered verifier.
