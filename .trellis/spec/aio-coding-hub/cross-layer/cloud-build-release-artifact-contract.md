...
- `scripts/check-local-native-boundary.mjs` fails closed on tracked hooks and
  ...
  JavaScript helpers are parsed with the pinned TypeScript AST and use explicit
  process contracts; any unconsumed process API reference, namespace, alias,
  optional/reflective call, dynamic child-process import, shell-enabled call, or
  otherwise unauditable dispatch fails. Cloud CI installs the parser dependency
  before the live boundary scan and runs self-tests only after that scan passes.
