...
- 两个 32px 数值列以 9px 等宽字体右对齐、tabular numbers、单行显示，确保五位原数和最长紧凑值都不溢出；数字变化只影响各自单元内部。
- `0..=99_999` 显示完整十进制数；`100_000..=99_999_999` 使用`万`，缩放值小于 100 时向下截断保留一位小数，否则向下截断为整数；`100_000_000..=u32::MAX` 使用`亿`并向下截断保留一位小数。所有紧凑值去掉末尾 `.0`，例如 `123456 -> 12.3万`、`999999 -> 99.9万`、`4294967295 -> 42.9亿`。
- 可见紧凑值只影响表现层；`aria-label` 与每个数字的 `title` 始终使用原始 `u32` 精确值。

紧凑格式化函数保持纯函数并单独测试 `99_999/100_000`、`999_999/1_000_000`、`99_999_999/100_000_000` 与 `u32::MAX` 边界，不改变 DTO、后端统计或排序。
