...
- 开工前提：阶段 B 先解决唯一预期的 `.trellis/tasks/README.md` 冲突并推送新 head，之后再以该 head 的自动检查作为交付证据。
...
- 执行 session：阶段 A session 已暂停；尚未创建阶段 B 执行 session。main 发送启动交接包后，执行 session 必须先解决唯一 README 冲突并推送新 head，之后才等待自动检查。

## 阻塞快照

- 无代码或失败 CI 阻塞；当前同步前的 README 冲突是阶段 B 的首个施工项。不得手动 dispatch、推空提交或修改工作流；解决冲突并推送新 head 后才等待实时自动检查。
...
- 上述云端证据属于归属迁移前的 #114 head 快照，不能作为当前候选；执行 session 解决唯一 README 冲突并推送新 head 后，main 再观察自动 CI，不手动触发工作流。
...
- 阶段 B 的同步、交付重绑和最新 head CI 复核尚未由执行 session开始；当前先按 `execution.md` 的启动条件解决 README 冲突，再等待自动检查。
...
- 接手条件：main 发送启动交接包后，一个执行 session 可写入本 worktree，仅同步 `origin/main` 并解决预期 README 冲突；其推送的新 head 自动检查绿色、head 未漂移后，才进入 main 验收交接。其余 session 保持暂停。
