...
- [x] 完成 AUD-056 代码提交 `1a733301`；PR、Actions、bindings 核验与合并列入待执行交付。
...
代码阶段统一执行：前序实现基线 -> failure-first 合同/测试源码 -> 最小实现 -> 本地允许检查 -> 独立五轴审查 -> 原子提交。交付恢复后统一执行：从最新主线提取对应提交 -> 重叠/漂移核对 -> 推送 -> 精确分支 `workflow_dispatch` -> 等待 `ci-gate` -> 最终主线门 -> 合并 -> 合并后树核验。PR 或 CI 不可用时记录待执行并继续下一项代码，不提前标记完成。
