...
- [ ] 提交、重放最新 `origin/main`、推送并创建面向 `main` 的唯一统一 PR。
- [ ] 对统一 PR 精确远端 head 触发 `ci.yml` 全量 `workflow_dispatch`，等待 `ci-gate`；仅接纳同一 SHA 的云端生成漂移。
- [ ] Actions 启动后按 #94、#93、#92、#87 顺序关闭旧 PR，并在评论中链接统一 PR。
- [ ] 合并统一 PR，核验主线树，再按完成证据收口仍在活跃列表的 PENDING/Trellis 状态。
