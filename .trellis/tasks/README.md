...
1. 采用 Trellis 的正式工作在活动目录中创建任务，并以 `task.json` 表达状态、父子关系和交付元数据。明确跳过 Trellis 的简单 main 任务改用 [月度 Markdown 变更记录](../../docs/operations/task-documentation-records.md)，不在这里创建空任务包。
...
3. 复杂任务在实施过程中同步维护 `implement.md`；轻量任务可以没有该文件。存在 `implement.jsonl`、`check.jsonl` 时，只允许它们引用实际存在的仓库文件。
