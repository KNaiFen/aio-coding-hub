...
2. 实施过程中同步维护 `implement.md`，并让 `implement.jsonl`、`check.jsonl` 只引用存在的仓库文件。
3. 任务完成且交付证据明确后，运行 `python3 ./.trellis/scripts/task.py archive --no-commit <task>`；不要手工移动或复制目录。
4. 归档后运行 `python3 ./.trellis/scripts/task.py validate --all`，确认活动和归档 manifest 全部有效。
5. 若任务产生长期有效的产品、架构或运维知识，应同步更新 [项目知识库](../../docs/README.md) 中的现行文档；不要要求读者从任务记录推断当前行为。
