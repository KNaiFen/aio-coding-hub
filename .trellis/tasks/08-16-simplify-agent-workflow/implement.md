...
- [x] 用 skill-creator 初始化并实现三个带 `aio-` 前缀的 repo skills，精确放行 `.agents/skills/**`，并用现有 source contract 固定名称。
...
- [ ] 完成最终验证：本地允许的 Node contracts、Node 语法、文档链接和全分支 `git diff --check` 已通过；两路独立只读验收发现已修复。Python unittest 和完整 CI 矩阵由 PR 最终 head 的 GitHub Actions 运行。
