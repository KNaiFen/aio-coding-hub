...
- 一句话结果：跨窗口协作改为短根规则、`aio-` 角色 skills、脚本化协调状态和分阶段文档，不再依赖模型手写多份 Git/worktree/验收状态。

## 实际实现

### 用户可见行为

- 新窗口通过 `$aio-trellis-main`、`$aio-trellis-execute`、`$aio-trellis-accept` 进入对应角色；执行窗口的 handoff 会显式调用带项目前缀的 skill。
...
| AC | 结果 | 证据 |
|---|---|---|
| AC-01 | 通过 | handoff 固定输出 primary folder、`$aio-trellis-execute`、status/doctor、任务入口；`doctor` 在当前 worktree 通过。 |
...
| AC-04 | 通过 | 三个 `.agents/skills/aio-trellis-*` 已被 Git 跟踪；cloud-only contract 固定精确 skill 名和默认 prompt 前缀。 |
...
| `.agents/skills/aio-trellis-*/SKILL.md` | main/execute/accept 三角色入口 | 新窗口按角色、按阶段加载，不依赖自定义 subagent 身份。 |
...
- 扩展现有链接/cloud-only 合同，而不是新建门禁；用于持续校验文档链接和 `aio-` skill 前缀。
...
| 本地 | `node scripts/check-cloud-only-verification.selftest.mjs` | 通过 | 包含破坏 `aio-trellis-main` 前缀的失败用例。 |
