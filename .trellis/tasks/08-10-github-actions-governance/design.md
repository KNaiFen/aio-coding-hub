...
`ci.yml` 继续监听 `push`、`pull_request` 和 `workflow_dispatch`。自动事件的聚合 job 保持 job id `ci-gate`，并将 job `name` 依据事件显示为 `ci-gate` 或 `manual-ci-gate`；这样 Ruleset 只匹配自动门禁。手动入口先运行无 checkout 的 `manual-dispatch-guard`，只有 `refs/heads/main` 才允许下游 jobs。
...
- `codeql.yml` 使用 `github/codeql-action` v4.37.6 固定 SHA `5595ccaf912efad79be6eef63a5619ff05969be3`，扫描 `javascript-typescript` 和 `rust`；PR、main/dev push、每周 schedule、手动运行，权限仅 `contents: read` 与 `security-events: write`，初期不加入 `ci-gate`。
