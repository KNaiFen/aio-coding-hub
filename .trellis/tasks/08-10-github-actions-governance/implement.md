...
3. **合同与文档**：同步 `ci-change-scope`、cloud-only/quality-gates checker 及 selftest；更新 README、AGENTS、现行 CI scope 合同和 cloud-only 合同。
4. **同步安全**：修改 `sync-upstream.yml` 和 policy checker/selftest；准备 App 权限与 secret 操作说明，不把私钥写入仓库。
5. **安全自动化**：添加 Dependabot、CodeQL、pin checker；将 pin checker 接入 support-contract；为全部既有 jobs 添加 timeout。
6. **远端设置**：代码 PR 合并且验证后，按顺序启用 SHA pinning、Dependabot/alerts/security updates，再把 Ruleset required contexts 改为 `ci-gate` + `pr-title`。App 创建/安装/私钥生成由仓库 owner 在 GitHub 设置中完成。
7. **Action 维护**：低风险 runtime action 单独升级；artifact pair、github-script、release action和 Tauri action 分别验证并可独立回滚。
