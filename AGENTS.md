...
## Main Worktree Session Handoffs

These requirements apply only when operating from the coordinator checkout (`main/`).

- After main creates and registers a sibling worktree for an independent execution task, proactively send the user an execution-session startup handoff. Do not assume that a task name, branch, or PR tells the user which directory contains the sole active writer.
- The handoff must state the task ID/goal and authoritative task directory; the absolute worktree path; branch, full `origin/main` base SHA, and planning commit; PR URL, current full head SHA, and CI state; current sole writer; whether work may start now; dependencies, out-of-scope files, prohibited actions, delivery/pause condition, and main's next review or notification gate.
- Include a prompt the user can paste into a new execution session. It must point to `execution.md`, repeat the absolute worktree path, branch, sole-writer constraint, and preflight check, and require the session to respect scope and stopping conditions, commit and push its work, update `delivery.md`, and pause after the latest head has passed the required CI. Do not duplicate the full design in chat or create a competing task specification.
- A startup decision is bound to one full PR head SHA. When the user, task plan, or candidate explicitly requires CI first, main monitors that head and only tells the user to start after the required checks are green, the head has not drifted, the worktree still exists, and its state is attributable. While CI is running, failed, unknown, or stale, or when the path is missing or dirty with unattributable changes, state "do not start" with the reason and next recheck condition.
- Pure planning-registration CI is not an implicit implementation gate. Main explicitly states whether CI must be awaited based on user authorization, the task plan, and risk rather than leaving that choice to the user or execution session.
- When the user reports paused sessions, verify that their paths remain registered worktrees for the current PR. A deleted, archived, merged, or unrelated path must be closed and never reused; only the path named in the current handoff may host a new or resumed writer.
- Task-worktree `prd.md`, `execution.md`, `delivery.md`, and task indexes contain only the goal, boundaries, prerequisites, verification, and delivery facts needed for the execution session's next action. CI polling, GitHub-event or conflict diagnosis, user communication, cross-worktree orchestration, and main coordination decisions belong in main's monthly change record or active-task index. The sole exception is a short actionable start/stop condition for the execution session, without the coordinator's investigation timeline.
