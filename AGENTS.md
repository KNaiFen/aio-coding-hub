...
- **main session**：使用 `$aio-trellis-main`。负责需求与方案、任务路由、worktree 生命周期、最终验收、合并、知识库/PENDING 和清理。
- **独立执行 session**：使用 `$aio-trellis-execute`。只在登记 worktree 施工，维护任务分支、PR、CI 和 `delivery.md`，交付后暂停。
- **验收 subagent**：使用 `$aio-trellis-accept`。只读返回证据，不写文件、PR 或状态，不合并、归档或清理。
...
- 不推送远端 `main`。执行 session 只推任务分支；只有 main 可以合并。
...
- 交付要求和阻塞恢复只读 [执行与交付](docs/operations/multi-worktree/execution-and-delivery.md)。完成后停止写入，等待 main 验收或明确返工。
...
- 验收必须针对暂停执行者后的实时 PR diff、固定 head 和 CI。只读 subagent 只提供线索，main 负责最终判断。
- `main-direct-fix` 仅限写权已登记转移、方案确定、任务范围内的记录性文档修正，且实时 scope 证明不会触发长检查；否则交回执行 session。
- 只有 main 可合并、archive、更新长期知识/PENDING、删除已合并且干净无占用的 worktree/branch。完整步骤由 `$aio-trellis-main` 按阶段加载。
