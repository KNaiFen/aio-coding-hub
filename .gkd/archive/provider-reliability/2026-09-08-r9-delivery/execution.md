> 归档快照：2026-09-08，来源 任务工作树 .gkd/execution.md。原作者决定和各轮观察时点保留；本轮后续交付事实见 [summary.md](summary.md)。个人路径与会话角色句柄已改为逻辑标识。
...
- Revision：r9-exec11，来源为已获批 PLAN r9-delivery；本轮修第五轮完整Rust结果中的四项失败。
- 用户批准：开始执行 PLAN。
...
此前writer/accept/monitor已停止，当前HEAD `2a01c75e19ff317200dc77e1710c6858e9161c0e`（在已审7d19a02e上仅云端fmt两hunk）。PR #192 run `34137431217` attempt1，rust job `101791619602` 完整测试结果2948 passed/4 failed/4 ignored，788.81s；fmt/clippy及frontend/macOS/contracts/pr-title成功，两个OAuth回归通过，一小时停顿已变成可读失败，不再猜测。

仅四项当前目标，保留全部原意和断言强度：

1. snapshot::candidates_keep_all_rows_when_details_are_truncated，2032行candidates.len=512、期望513；snapshot::fixed_and_all_provider_views_reuse_spend_after_the_source_is_unavailable，1967行候选=[]、期望[codex]。main核对insert_observer_provider只upsert，没有插入default_route_providers，而load_provider_candidates确实只读active mode路由成员。批量512个已显式插入，首个和fixed测试codex未显式加入。确认实际upsert语义后，仅在这两fixture显式建立所需路由身份/启用，保留513/512和DROP VIEW后复用/limited断言；不要把生产候选改成全供应商或降低期望。
2. domain/provider_limit_usage.rs::filtered_cost_projection_preserves_compatibility_and_complete_backfill_totals，835行原SQL与新SQL一致，但false分支总额14而期望15。main已核对新空DB迁移v42_to_v43初始将status标记complete；fixture的false分支仅DELETE ledger99未把status改incomplete。显式设正确回填状态覆盖真实兼容视图，再切complete；保留原SQL等价、零金额、5窗口和物化查询计划断言，不改生产视图/SQL来修fixture。
3. probe runtime::shared_deadline_survives_gateway_and_circuit_lock_waits明确在 `deliver timeout with real lock held: manager=true,before_configuration=false,change_configuration=false` 触发30秒真实诊断。main已核对finish的blocking closure在manager前释放inner，expire_probe只短inner/预算证据；暂未见确定同步锁环路。该fixture手动建立current_thread runtime后pause，advance恰好60秒；tokio timer有粒度/注册唤醒边界，其他 #[tokio::test(start_paused=true)]回归已通过。请先证明deadline确已到达、timeout future在同clock被驱动，以及发送/两个waiter分别阶段（不要用总phase继续掩盖原因）。可针对fixture给timer tick明确的最小推进（例如实际deadline再一个timer粒度），必须同时断言预算恰为60秒、过期状态，不能改变生产预算或把持有真实锁提前释放。若是实际生产协调阻塞则在原exec10允许probe runtime/domain/provider queries/gateway/circuit探测专用边界最小修复且提供可证明链路；不得无证据重构。保留真实manager/circuit四矩阵、双等待方、again admission、超时失败一次、半开恢复顺序；补必要诊断要局部并失败释放。

写范围：snapshot.rs上述测试、domain/provider_limit_usage.rs上述测试、provider_availability_probe_runtime.rs锁回归及仅有证据需要的原探测实现边界；production更改必须说明原因。不要修改已通过OAuth/其他回归或CI工作流。本地只Git/源码/git diff --check，禁测试/Cargo/Node/格式工具/构建/安装；保持现有fmt风格。分项中文提交、消融、progress准确记录后停止，不推送/验收/发布/派生代理。

### r9-exec10 唯一当前返工

前轮writer/accept/monitor均停止，当前HEAD `f6bbcc82720c1e75ef03b54a89a1a96de76458c0`。PR #192 自动run `34128761259` attempt1，Rust job `101763609700`：fmt/lock/bindings无漂移，clippy通过。frontend/contracts/observer-macos/pr-title成功。Rust tests编译后于13:51:05Z开始2956项，以下四项FAILED；14:00:49Z最后完成shared_deadline_releases...后没有新测试结果，到14:41:28Z因job一小时硬限被取消。GitHub annotation明确 `The job has exceeded the maximum execution time of 1h0m0s`。由于libtest捕获，失败详情没有刷出；不把这个超时作为加大CI预算理由。

本轮修复范围内新增回归及必要产品实现，不能删测试/断言或绕过预算。准确目标：

1. `app/observer/snapshot.rs::tests::fixed_and_all_provider_views_reuse_spend_after_the_source_is_unavailable` FAILED：main看到1937行借单连接conn后，1943行仍持它调用list_v1，后续load也未drop。最小修正持有期，保留DROP VIEW后无重复汇总、固定/all/空范围和limited断言。
2. 同文件 `candidates_keep_all_rows_when_details_are_truncated` FAILED：检查批量INSERT fixtures与当前schema约束、route成员ID，保留513候选与512详情及限额断言。不要修改生产schema或放宽唯一约束。
3. `app/provider_availability_probe_runtime.rs::tests::oauth_acceptance_orders_timeout_and_configuration_before_commit` 与 `accepted_oauth_commit_orders_real_reads_without_blocking_admission_or_slots` FAILED，时间均不足1秒，不能仅按慢CI猜测。静态核对真实mock refresh路径/字段、headers、test override、DB连接及通知到达条件，修正可证明失败；保持接受前后/配置竞态、真实读取在旧commit前推进、普通WAL对照、33轮及无重复refresh所有原行为断言。
4. `shared_deadline_survives_gateway_and_circuit_lock_waits` 是按单线程测试名称排序在最后ok后应执行的下一项，疑似阻塞但日志尚未证明开始。main已读其完整体：独立current_thread clock、tauri异步runtime、std线程持manager/circuit lock、pause注入、advance60、完成消费及新flight。核对生产锁/fixture同步关系，找出不能到达ready或completion的路径，修复实际原因；必须保留真实manager/circuit等待超时释放、before_configuration/change_configuration矩阵、超时失败消费一次和半开恢复次序，不以减弱断言/释放旧锁后才超时制造通过。若观察到生产问题在已批准的probe runtime/domain/providers/queries、gateway_state/gateway runtime/circuit专用探测边界内，允许必要最小修复及回归；超出原行为或这些边界返回main。

允许写入上述snapshot测试、probe runtime及必要原范围内 `domain/provider_availability.rs`、`domain/providers/queries.rs`、`app/gateway_state.rs`、`gateway/runtime.rs`、`shared/circuit_breaker.rs` 的probe专用部分；优先仅fixture，生产修改须证据与消融记录。原OAuth适配器仅只读依赖，不改正常OAuth协议。为后续云端及时暴露这些失败，可在上述新测试关键等待处加有限且正确时钟的诊断失败边界（释放持锁线程后再panic，不能阻塞runtime销毁），或限定这些测试输出必要合成阶段标识；不要修改常规CI命令/timeout/门禁，不记录敏感数据、不引入通用诊断框架。

本地仍禁止测试、Cargo、Node脚本、package-manager、格式/构建，仅Git/源码、git diff --check。按修复职责分项中文提交，准确progress记录已证明/仍推断及未跑运行验证，消融后停止。若确实缺少错误细节以决定产品修复，先完成可证明的fixture和最小范围测试诊断，不瞎改；返回main推进新自动CI。你不推送、验收、合并发布或派生代理。

### r9-exec9 唯一当前返工

本节覆盖全部历史返工。上一writer及accept已停止，当前HEAD `c1c002c5556054ec1b43ff2de22bb1dc863a824b`。自动run `34127583409` 的rust job `101759838184` canonicalize无漂移，clippy失败；contracts/pr-title成功，frontend/macOS在main取证时仍运行。只修这四处：

- `src-tauri/src/app/provider_availability_probe_runtime.rs:380` 的Lead异步work在尾部 `if budget.expired() { return; } else if ...` 触发needless_return。使用最小等价控制流，继续保证过期结果不complete/finish；不要留空条件块或增加warning豁免，不修改完整预算/消费顺序。
- `src-tauri/src/domain/provider_availability.rs:2813,2848,2883` 三个本任务raw HTTP mock服务器直接丢弃 `socket.read` 返回字节数，触发unused_io_amount。按真实测试意图处理读取；这些是仅接收简单GET后发送预设损坏/边界响应的fixture，应确保实际收到请求，保留响应边界、提前EOF、读超时、终态早返回及耗时所有原断言。不要用read_exact填满1024缓冲造成不可能完成的等待，不添加无关通用HTTP解析框架、不局部allow绕过clippy。

只允许上述两文件和自己的progress，保持云端fmt风格；本地仍只Git/源码及git diff --check，禁止本地测试/Cargo/Node检查/构建/格式工具。不改变质量门禁，不推送。一个具体中文Conventional Commit，消融、progress、完整HEAD和静态验证后停止，不派生代理。

### r9-exec8 唯一当前返工

本节覆盖下方历史返工清单。此前所有 writer/accept/CI monitor 均已停止。PR #192 为 OPEN，初始 head `37513933403682f68aad0c135c0ca98220ed8fb1` 的自动 run `34125500658` attempt 1 失败。main 已核对该 run 的 cloud-native-fixes 补丁，确认其 merge SHA 的父提交包含本 PR head；应用 Rust fmt、Cargo.lock 三个0.60.59版本与objc2直接依赖，以及bindings两个可选模型字段，提交 `aff75018`。从此提交继续，保留所有原提交及云端格式。

1. contracts 失败于 `ci.yml observer-macos must retain the approved fail-closed script`。main 已定位 `scripts/check-ci-quality-gates.mjs::requireWorkflowRunScript` 仅接受 `run: |` 块；`.github/workflows/ci.yml` 新 observer-macos 测试步使用单行 run。最小修正工作流表示，使其保持原已批准的确切 cargo 命令、30分钟、macos-latest和门禁，不放宽校验函数或删除负例。必要核对/补充只限已有两个check-ci-quality-gates脚本。
2. frontend 2866个测试中仅 `ProviderRoutingEditor.test.tsx` 的 `keeps a dirty cross draft when its source member becomes disabled` 失败，586行找不到 `local-draft`。之前的save调用参数断言已通过，说明草稿至少已送入保存；同scope普通refetch用例通过。调查真实编辑器的禁用展示与失败保存生命周期：究竟是草稿被丢弃，还是停用状态隐藏输入导致测试使用错误可见性断言。保留原用户要求：脏草稿不能静默丢弃、不能误报保存成功、不能修改停用资格。最小修正必要源码或测试；若仅隐藏，覆盖错误提示/未关闭未成功，并在重新启用同成员或真实下一次保存时证明草稿仍存在，不能简单删掉保留断言。允许写入 `src/pages/providers/useProviderEditorForm.ts`、该测试；`ProviderEditorDialog.tsx` 及其cross routing组件为只读依赖。确有产品展示修复必要再带准确证据返回main。

只修改上述范围及自己的progress；不要重做历史实现、修改版本或其他Rust文件。未跑的 clippy/Rust测试/macOS仍待下一轮云端；不要把格式生成成功当作编译回归成功。本地仍仅Git/源码和git diff --check，不运行任何Node脚本、测试、构建、Cargo或package-manager。分两项中文Conventional Commit，完成消融并更新progress后停止，不推送、验收、合并、发布或派生代理。
