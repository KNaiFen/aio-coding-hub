...
- **状态**：`planned`
- **日期**：2026-08-06
- **待执行交付**：代码候选已在独立分支 `codex/aud020-cross-restart-data-reset` 提交为 `5ab4e48b`；本地仅通过零依赖 cloud-only checker/self-test 与 `git diff --check`，未运行 Cargo、pnpm、Tauri、生成器、类型检查、Lint、测试或构建。PR、精确 head 全量 CI、跨平台故障注入与主线门统一后置；在云端验证和合并前保持 `planned`。
