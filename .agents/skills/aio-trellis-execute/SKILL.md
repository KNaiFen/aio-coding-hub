...
- When ready, commit and push all task changes, make the PR reviewable, report the latest head/CI link in the handoff message, and pause.

Do not spawn another implementation chain, merge, enable auto-merge, write main acceptance, archive, update PENDING history, or remove the worktree. Resume only after main updates the persistent state and explicitly returns write ownership.
